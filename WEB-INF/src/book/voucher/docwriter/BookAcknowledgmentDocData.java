...
public class BookAcknowledgmentDocData implements DocData {

	private static final long serialVersionUID = 7303881761692601117L;

	private String nendo;
	private String from;
	private String to;

	/** 行データ(key:領収書番号、value:RowData **/
	private Map<String, RowData> rowDataMap = new LinkedHashMap<String, BookAcknowledgmentDocData.RowData>();
	...
	void setReceiptDate(String key,String receiptDate) {
		createRowData(key);
		rowDataMap.get(key).setReceiptDate(receiptDate);
	}

	public String getReceiptDate(String key) {
		return (rowDataMap.get(key) == null)
			? null : rowDataMap.get(key).getReceiptDate();
	}

	void setReceiptNo(String key,String receiptNo) {
		createRowData(key);
		rowDataMap.get(key).setReceiptNo(receiptNo);
	}

	public String getReceiptNo(String key) {
		return (rowDataMap.get(key) == null)
			? null : rowDataMap.get(key).getReceiptNo();
	}
	...
	private class RowData {
		...
		private void setReceiptNo(String receiptNo) {
