...
<td class="td02" style="width:90px;"><html:text styleId='dealCode' property='dealCode' name='BillForm' size='20' maxlength='10' onblur='if(!checkDealCode(this.value)) { this.focus(); } else { searchDeal(this.value); }'/></td>
</tr>
...
<input type="button" id="deal" value="<bean:message key="COMMON.BTN.DEAL"/>" onclick="openBillCustomerDialog('');">
<html:radio styleId="dealKindCustomer" property="dealKind" name="BillForm" value="<%= ViewProperties.getInstance().getValue(ViewProperties.DEAL_KIND_CUSTOMER, ViewProperties.VALUE) %>"><bean:message key="COMMON.LABEL.CUSTOMER"/></html:radio>
<html:radio styleId="dealKindSupplier" property="dealKind" name="BillForm" value="<%= ViewProperties.getInstance().getValue(ViewProperties.DEAL_KIND_SUPPLIER, ViewProperties.VALUE) %>" disabled="true"><bean:message key="COMMON.LABEL.SUPPLIER"/></html:radio>
</td>
</tr>
